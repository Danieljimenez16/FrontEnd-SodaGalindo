...
function Dashboard({ onLogout }) {
  ...
  // Cargar resúmenes y mapear _id a id
  ...
  const totalGastos = Number(form.municipalidad) + Number(form.facturas) + Number(form.mesada) + Number(form.salarios);
  const gananciaFinal = Number(form.ventas) - totalGastos;

  const totalGananciasAll = resumenes.reduce((acc, r) => {
    const rTotal = r.totalGastos ?? (Number(r.municipalidad) + Number(r.facturas) + Number(r.mesada) + Number(r.salarios));
      ...
      <div className="table-container">
        ...
        <div className="table-wrapper">
          <table>
            ...
            <tbody>
              {resumenes.map((r, idx) => {
                const rowTotal = r.totalGastos ?? (Number(r.municipalidad) + Number(r.facturas) + Number(r.mesada) + Number(r.salarios));
